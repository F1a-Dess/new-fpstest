...
const directionalLight = new THREE.DirectionalLight( 0xffffff, 80 );
...
const pLight = new THREE.PointLight(0xc4c4c4, 0.6);
...
const playerCollider = new Capsule( new THREE.Vector3( 0, 0.3, 0 ), new THREE.Vector3( 0, 1, 0 ), 1);
...
function playerSphereCollision( sphere ) {
    ...
    const center = vector1.addVectors( playerCollider.start, playerCollider.end ).multiplyScalar( 0.1 );
...
function controls( deltaTime ) {
    ...
    const speedDelta = deltaTime * ( playerOnFloor ? 25 : 5 );
...
function teleportPlayerIfOob() {
    ...
    if ( camera.position.y <= - 25 ) {
        ...
        playerCollider.start.set( 0, 12, 0 );
...
function newAnim() {
    if(mixer){
        ...
        mixer.update( newClock.getDelta() );
    }
    renderer.render(scene, camera);
};

renderer.setAnimationLoop(newAnim);
